...
public sealed class WorkbenchDatabaseTests
{
    ...
    [Fact]
    public async Task Latest_migrations_set_user_version_to_20()
    {
        await using var temporary = new TemporaryDatabase();
        var database = new WorkbenchDatabase(temporary.DatabasePath);
        ...
        Assert.Equal(20L, await command.ExecuteScalarAsync());
    }

    [Fact]
    public async Task Migrating_v11_database_to_v20_preserves_continuity_data_and_is_idempotent()
    {
        await using var temporary = new TemporaryDatabase();
        ...
        Assert.Equal(20L, firstSnapshot.UserVersion);
