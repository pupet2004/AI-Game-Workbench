...
public sealed class TaskRepository(WorkbenchDatabase database)
{
    ...
    public async Task<IReadOnlyList<StoredTask>> ListDraftsAsync(Guid projectId, CancellationToken ct = default) =>
        (await ListAsync(projectId, ct)).Where(item => item.Status == TaskLifecycleStatus.Draft).ToArray();
    public async Task<bool> UpdateStatusAsync(Guid projectId, Guid taskId, TaskLifecycleStatus expectedStatus, TaskLifecycleStatus status, CancellationToken ct=default){if(!AssignmentLifecycle.CanTransition(expectedStatus,status))throw new ArgumentException($"{expectedStatus} cannot transition to {status}.",nameof(status));await using var c=_database.CreateConnection();await c.OpenAsync(ct);var q=c.CreateCommand();q.CommandText="UPDATE tasks SET status=$s,updated_at=$at,cancelled_at=$ca WHERE project_id=$p AND id=$t AND status=$expected";q.Parameters.AddWithValue("$s",status.ToString());q.Parameters.AddWithValue("$at",DateTimeOffset.UtcNow.ToString("O"));q.Parameters.AddWithValue("$ca",status==TaskLifecycleStatus.Cancelled?DateTimeOffset.UtcNow.ToString("O"):(object)DBNull.Value);q.Parameters.AddWithValue("$p",projectId.ToString());q.Parameters.AddWithValue("$t",taskId.ToString());q.Parameters.AddWithValue("$expected",expectedStatus.ToString());return await q.ExecuteNonQueryAsync(ct)==1;}
    public async Task UpdateTitleAsync(Guid projectId,Guid taskId,string title,CancellationToken ct=default){ArgumentException.ThrowIfNullOrWhiteSpace(title);await using var c=_database.CreateConnection();await c.OpenAsync(ct);var q=c.CreateCommand();q.CommandText="UPDATE tasks SET title=$title,updated_at=$at WHERE project_id=$p AND id=$t AND status='Draft'";q.Parameters.AddWithValue("$title",title);q.Parameters.AddWithValue("$at",DateTimeOffset.UtcNow.ToString("O"));q.Parameters.AddWithValue("$p",projectId.ToString());q.Parameters.AddWithValue("$t",taskId.ToString());await q.ExecuteNonQueryAsync(ct);}
}
