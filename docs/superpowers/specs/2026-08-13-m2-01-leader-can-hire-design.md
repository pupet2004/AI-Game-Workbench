...
`User goal -> Leader creates Draft Task -> user reviews/edits -> Start Worker -> freeze revision/BaseCommit/TargetBranch/ExecutionProfile -> create branch/worktree -> start Worker AgentSession -> Worker edits and may commit -> Workbench records events/evidence -> Final Report -> CompletedPendingReview`.
...
- Reproducibility is defined by the immutable Task Revision, frozen full `BaseCommit`, frozen `ExecutionProfile`, `TargetBranch`, and owned Worker branch/worktree.
...
The execution record binds exactly one Task Revision, BaseCommit, TargetBranch, ExecutionProfile, WorkerBranch, WorkerWorktreePath, and Worker Session identity. A Running execution must have all of these fields plus a successfully started session.

### ExecutionProfile

Three separate concepts are frozen together at Start: `Provider`, `ModelProfile`, and `AgentRuntime` (for example, GPT-5.6 via Codex). The contract remains provider-neutral and can later represent other providers/runtimes.

### Task Packet

Structured input to the Worker containing TaskId, revision, complete contract, BaseCommit, TargetBranch, WorkerBranch, WorkerWorktreePath, frozen profile, permission boundaries, all relevant Formal Project Memory, and a small bounded selection of relevant Learned Memory. Project Memory authority is explicit: `Formal > Learned`; a normalized Formal topic suppresses a Learned item on that topic. Pending Candidate, Rejected, and Superseded memory are excluded. M2-01 uses deterministic bounded selection only; no semantic retrieval. Full Leader transcript, all Worker histories, all Project Memory, and all Library content are not injected by default.
...
`Start Worker` is the only transition that begins execution and requires explicit user action. That action simultaneously approves execution, freezes the current BaseCommit and ExecutionProfile, and grants Level 2 capability only within the current Task and Worker worktree. Material changes to Goal, Scope, Acceptance, or an architecture/product decision require a new user-approved Task Revision; the old snapshot is never overwritten. Non-material terminology or boundary explanations are interactions/events only and do not create a revision.

When a new revision is created, the Worker must emit an explicit `TaskRevisionAck` before it is treated as using that contract. Evidence and Final Report must identify `ExecutedAgainstRevision`. Leader clarification may explain the existing contract using Task and certified Project Memory, but may not silently rewrite it. Material ambiguity is escalated to the user.
...
4. Freeze the selected ExecutionProfile and current Task Revision.
...
The Draft contains a recommendation. The user may edit it. At Start, the chosen Provider, ModelProfile, and AgentRuntime are persisted as an immutable execution freeze. Workbench-level approval is distinct from provider/runtime approval; any provider-specific prompt must be mapped through Workbench policy and existing Task grants, never blanket auto-approved.

## 10. Worker Session / Resume

Persistence must retain enough identity to resume: Provider, Account, Model, Runtime selection, AgentSessionId, ExternalSessionId, WorkingDirectory, TaskId, and Revision. On interruption (runtime crash, provider/network interruption, Workbench restart, or process loss), prefer resuming the same AgentSession, worktree, branch, and Task Revision. If the runtime explicitly cannot resume, stop and request a user decision; do not silently create a replacement session or switch brains. Cross-session continuation is out of scope.
...
`TaskClarificationRequest` is distinct from a PermissionRequest and means the Worker cannot determine what the Task contract requires: goal ambiguity, scope/acceptance conflict, BaseCommit contradicting assumptions, or a necessary architecture decision. It routes first to Main Leader. The Leader may clarify terms and existing boundaries from the Task and certified Formal/Learned Memory, but cannot change Goal, Acceptance, materially expand Scope, or decide architecture/product direction on the user's behalf. Material changes require a new user-approved revision.
...
- `Running -> Blocked`: a PermissionRequest, TaskClarificationRequest, User Decision, or required Revision ACK blocks a critical path. Blocked is not Failed.
...
- `Interrupted -> Running`: same-session resume succeeds with same worktree, branch, and revision.
- `Running -> CompletedPendingReview`: Worker submits Final Report and Workbench collects deterministic evidence.
- `Running` or `Interrupted` -> `Failed`: only after simple Resume cannot continue and the user/Leader explicitly terminates or the Worker is demonstrably unable to satisfy Acceptance.
...
Workbench independently collects deterministic evidence: BaseCommit, Worker final HEAD, Worker branch, Worker worktree, changed files, `git diff --stat`, `git status`, and commit list. These facts cannot be accepted solely from Worker prose. Structured test/build reruns and verdicts are M2-02 concerns.
...
| Revision pending ACK | Worker remains blocked against old contract; no new-contract evidence until explicit ACK. |
| Runtime interruption | Enter Interrupted; preserve session/worktree/branch/revision; allow same-session Resume. |
| Resume failure | Do not auto-replace Worker; request user decision and allow explicit termination to Failed. |
...
Start atomicity invariant: no externally visible `Running` execution exists without complete frozen configuration, owned branch/worktree, and (once started) Worker Session identity. Recovery may reconcile durable intermediate records without inventing a different execution identity.
...
Persist immutable Task Revisions and their approval metadata; current Task lifecycle; execution freeze (Revision, BaseCommit, TargetBranch, ExecutionProfile); branch/worktree ownership; Worker Session identity; Task/Worker Events; PermissionRequests and Task-scoped grants with expiry; Clarification interactions; Final Report; deterministic evidence; and archive references. Preserve worktrees/branches through CompletedPendingReview. Design indexes/queries so Leader Inbox and Active/Pending views are bounded by actionable status/time rather than total Task count. No task-vN files, project copies, or migration design is part of this spec.
...
- BaseCommit, TargetBranch, Revision, and ExecutionProfile freeze at Start.
...
- Worker explicitly ACKs a new Revision before executing against it.
- Interrupted resumes the same Session/worktree/branch/revision or asks the user.
