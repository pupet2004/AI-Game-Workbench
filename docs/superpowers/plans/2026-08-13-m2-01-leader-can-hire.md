...
- `Start Worker` is explicit user action. Draft creation never freezes BaseCommit, creates a worktree, starts a runtime, or consumes Worker runtime.
...
**Schema strategy:** Add one Migration007 after v6. Use eight tables, not one table per event subtype: `tasks`; `task_revisions`; `worker_executions` (includes frozen identity, `ExecutionStartRevision`, `CurrentAcknowledgedRevision`, WorkerBranch, WorkerWorktreePath, startup/recovery state, AgentSessionId, ExternalSessionId, and final state); `task_events`; `permission_requests`; `task_grants`; `task_clarifications`; and `worker_completion_packages` containing Final Report and deterministic Evidence payloads. Add foreign keys to `projects`, `tasks`, and revisions, unique `(project_id, active_write_execution)` enforcement through a partial unique index or equivalent guarded update, indexes on actionable execution/event status and `(project_id, created_at)`, and a unique `(task_id, revision_number)`. Preserve existing Project, Leader, Memory, and settings tables unchanged.
...
- [ ] Add repository tests for recoverable states (`Preparing`, `WorkspaceCreated`, `RuntimeStarting`, `Interrupted`) and safe restart reads.
...
## Task 3: Git Worktree Ownership and Deterministic Command Boundary
...
## Task 4: Start Worker Coordinator and Cross-System Recovery
...
**Start sequence:** `clean main worktree check -> one active write Worker check -> freeze Git/runtime identity -> create branch/worktree -> durably bind workspace -> create Worker AgentSession -> persist AgentSessionId/ExternalSessionId -> Running`.

**Recovery rule:** SQLite transactions protect each durable transition, but never pretend to cover Git/runtime side effects. On startup, coordinator reconciliation examines `Preparing`, `WorkspaceCreated`, and `RuntimeStarting`: verify ownership and expected branch/worktree, complete a missing durable bind when unambiguous, remove only identifiable partial branch/worktree artifacts, retain a diagnosable workspace after runtime creation failure, and surface user decision for ambiguous cases. Never auto-create a replacement Worker session.
...
## Task 5: Task Packet and Bounded Project Memory
...
## Task 6: Provider-Neutral Worker Session Create, Send, Interrupt, and Resume
...
## Task 7: Permission Policy, Task Grants, Clarifications, and Revision ACK
...
## Task 8: Worker States, Task Events, Archive References, and Leader Inbox
...
## Task 9: Final Report, Deterministic Evidence, and CompletedPendingReview
...
## Task 10: Minimal UI Slice and Explicit User Controls
...
## Task 11: Cross-Layer Regression and Real Worker Smoke
...
- [ ] Write failing integration coverage for Draft no side effects, dirty Start rejection, BaseCommit/Profile/account freeze, one active Worker per Project, isolation, partial-start recovery, same-session resume, permissions, clarifications, revision ACK, bounded Inbox, no progress flooding, report/evidence separation, retained worktree, and unchanged main worktree.
...
- **Core:** immutable revision pointers, execution identity, grant boundaries, clarification/Failed authority.
- **Storage:** v7 upgrade, foreign keys, repositories, CAS ACK, durable state/event/request/report/evidence records, project isolation.
...
- **Runtime:** existing provider-neutral contracts, create/send/resume/interrupt, same account/session, no Codex coupling in Worker core.
- **App:** coordinator sequencing/recovery, packet memory bounds, policies, Inbox bound, completion, UI controls, regression with existing Leader boot/rollover.
- **Real smoke:** opt-in disposable safe repository and real Codex Worker only after fake/integration suites are green.

## Commit Strategy

Each implementation task produces one focused, reviewable commit in order: core contracts; v7 storage; Git worktree isolation; recoverable Start coordinator; packet builder; runtime session lifecycle; permissions/clarifications/ACK; events/Inbox; completion/evidence; UI; end-to-end smoke/stabilization. No squash-all-M2-01 commit and no M2-02 commit.

## Spec Coverage

Missing: none. The plan maps Draft/Start, dual revision pointers, frozen Git/runtime identity, dirty and concurrency guards, worktree isolation, bounded memory packet, provider-neutral runtime, permissions, clarifications, ACK, interruption/resume, event/archive/Inbox bounds, report/evidence, CompletedPendingReview retention, minimal UI, and M2-02 exclusion to concrete tasks and tests.

## Self Review

Placeholders: none.

Type inconsistencies: none. `ExecutionStartRevision` and `CurrentAcknowledgedRevision` are used consistently across Core, Storage, Coordinator, Resume, Evidence, and tests.

Dependency ordering: Tasks 1–3 establish contracts/storage/Git; Task 4 composes Start recovery; Tasks 5–9 add packet/runtime/policy/events/completion; Task 10 binds UI; Task 11 performs cross-layer and real smoke verification.
...
The existing runtime API already provides provider-neutral create/resume/send/stop primitives and account-scoped registry lookup, so no runtime contract expansion is assumed. Exact branch naming and cleanup command details remain implementation choices constrained by the ownership and recovery invariants above; they do not block this plan.
