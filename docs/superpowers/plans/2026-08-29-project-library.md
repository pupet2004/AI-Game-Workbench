...
**Goal:** Present Project Library as category, time, project, and overview projections using existing durable data.

**Architecture:** Keep storage and authority contracts unchanged. Extend the Library pane view model with selected category/date state and daily summary/summary-entry projections, then replace the flat Avalonia templates with explicit index/detail regions.
...
- [x] Add selected category/object/date state and read-only view records.
...
- [x] Render time date index and selected date detail separately.
- [x] Put Daily Summary and decisions before linked Library nodes.
